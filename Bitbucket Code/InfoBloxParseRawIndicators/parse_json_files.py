...
class ParseJsonFiles:
    ...
    def return_file_names_to_parse(self, files_list: list):
        """Return list of file names to parse.
        ...
        try:
            ...
            for file_name in files_list:
                ...
                if (len(file_name_split) > 5) and (file_name_epoch + consts.ONE_HOUR_EPOCH_VALUE < current_epoch):
                    list_of_files_to_parse.append(file_name)
            ...
            applogger.info(
                consts.LOG_FORMAT.format(
                    ...
                    "No. of files to be parsed = {}".format(list_of_files_to_parse),
                )
            )
    ...
    def list_file_names_in_file_share(self):
        """Get list of file names from directory (File share).
        ...
        __method_name = inspect.currentframe().f_code.co_name
        try:
            files_list = list(self.parent_dir.list_directories_and_files(consts.FILE_NAME_PREFIX))
            file_names = []
            if (len(files_list)) > 0:
            ...
            applogger.info(
                consts.LOG_FORMAT.format(
                    ...
                    __method_name,
                    consts.PARSE_RAW_JSON_DATA_FUNCTION_NAME,
                    "No. of files in file share = {}".format(len(files_list)),
                )
            )
            return file_names
            ...
            applogger.error(
                consts.LOG_FORMAT.format(
                    ...
                    __method_name,
                    consts.PARSE_RAW_JSON_DATA_FUNCTION_NAME,
                    "Error while getting list of files: {}".format(error),
                )
            )
            raise InfobloxException()
    ...
    def get_checkpoint_data(self, file_name):
        ...
        try:
            ...
            # !this raise was not here before
            raise InfobloxException()
    ...
    def replace_raw_file_with_completed(self, file_name, data):
        ...
        try:
            ...
            applogger.info(
                "{}: (method = {}) : {} : file name to post = {}".format(
                    consts.LOGS_STARTS_WITH,
                    __method_name,
                    consts.PARSE_RAW_JSON_DATA_FUNCTION_NAME,
                    new_file_name,
